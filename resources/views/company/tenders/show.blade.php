...
@section('content')
    <div class="container-fluid p-4">
                        ...
                        <div>
                            ...
                            <div class="d-flex flex-wrap gap-4 mb-3">
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <span>{{ __('Created') }}
                                        {{ Carbon::parse($tender->created_at)->format('M d, Y') }}</span>
                                </div>
                            </div>
                        </div>
                        <div>
                            ...
                            @if ($tender->end_date > now())
                                <form action="{{ route('stopTender', $tender->id) }}" method="post">
                                    @csrf
                                    <button class="btn btn-danger {{ $tender->status == 0 ? 'disabled' : '' }}"
                                        type="submit">
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                                ...
                                <div class="info-section">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <p class="mb-0">
                                                {{ \Carbon\Carbon::parse($tender->end_date)->format('M d, Y') }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                @if (auth()->user() &&
                        (auth()->user()->role == 'admin' || auth()->user()->role == 'admin_company' || $tender->end_date < now()))
                    <div class="content-card">
                        <div
                                ...
                                <div class="applicant-card p-3 border-bottom">
                                    <div class="row align-items-center">
                                        ...
                                        <div class="col-12 col-md-6 col-lg-4 mb-3 mb-lg-0">
                                            <div class="d-flex align-items-center">
                                                <div class="applicant-avatar me-3 text-center bg-primary text-white rounded-circle"
                                        ...
                                        <!-- Files -->
                                        <div class="col-12 col-md-6 col-lg-6 mb-3 mb-lg-0" style="display: flex;gap: 15px;">
                                            ...
                                            @if ($tender->status == false || \Carbon\Carbon::parse($tender->end_date)->lt(\Carbon\Carbon::today()))
                                                @if ($applicant->pivot->quantity_file)
                                                    @php
                                                        $fileArray = array_filter(
                                                            explode(',', $applicant->pivot->quantity_file),
                                                        );
                                                    @endphp
                                                    <div class="d-flex flex-wrap">
                                                        @foreach ($fileArray as $file)
                                                ...
                                                @if ($applicant->pivot->financial_file)
                                                    @php
                                                        $fileArray = array_filter(
                                                            explode(',', $applicant->pivot->financial_file),
                                                        );
                                                    @endphp
                                                    <div class="d-flex flex-wrap">
                                                        @foreach ($fileArray as $file)
                ...
                @if ($tender->qr_code)
                    <div class="content-card">
                        <div class="card-body p-4 text-center">
                            ...
                            <div class="tender-id mb-3">
                                <strong id="tenderId">{{ $tender->id }}</strong>
                            </div>
                            <div class="tender-name mb-3">
                                <strong id="tenderName">{{ $tender->title }}</strong>
                            </div>
                            <div class="tender-created-at mb-3">
                                <strong
                                    id="TenderCreated">{{ Carbon::parse($tender->created_at)->format('M d, Y') }}</strong>
                            </div>
    ...
    @push('scripts')
        <script>
                ...
                function updateSimpleCountdown(distance) {
                    ...
                    if (days > 0) {
                        simpleText.textContent =
                            `${days} ${days === 1 ? 'day' : 'days'} ${hours} ${hours === 1 ? 'hour' : 'hours'} remaining`;
                    } else if (hours > 0) {
            ...
            function downloadQR() {
                const qrSvg = document.querySelector('.qr-container svg');
                const tenderName = document.getElementById('tenderName')?.textContent.trim() || 'tender-qr';
                const tenderId = document.getElementById('tenderId')?.textContent.trim() || 'tender-id';
                const TenderCreated = document.getElementById('TenderCreated')?.textContent.trim() || '';
                ...
                const textHeight = 200; // Increased to fit 3 lines
                ...
                img.onload = function() {
                    ...
                    ctx.fillText(`ID: ${tenderId}`, canvasSize / 2, canvasSize + 40);

                    // Draw tender name
                    ctx.font = 'bold 36px Arial';
                    ctx.fillText(tenderName, canvasSize / 2, canvasSize + 90);

                    // Draw tender created date
                    ctx.font = 'normal 32px Arial';
                    ctx.fillText(TenderCreated, canvasSize / 2, canvasSize + 140);
                    ...
                    canvas.toBlob(function(blob) {
                        ...
                        const a = document.createElement('a');
                        a.href = url;
                        a.download = `tender-${tenderId}-${tenderName.replace(/\s+/g, '_')}-qr.png`;
                        document.body.appendChild(a);
                        a.click();
                        document.body.removeChild(a);
                        URL.revokeObjectURL(url);
