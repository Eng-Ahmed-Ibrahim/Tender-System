                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="title_ar" class="form-control" value="{{ $tender->title_ar }}" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="title">{{ __('First Insurance')}}</label>
                            <input type="number" name="first_insurance" class="form-control" value="{{ $tender->first_insurance }}" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="title">{{ __('Price')}}</label>
                            <input type="number" name="price" class="form-control" value="{{ $tender->price }}" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="title">{{ __('City')}}</label>
                            <input type="text" name="city" class="form-control" value="{{ $tender->city }}" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="edit_end_date" class="form-control" value="{{ $tender->edit_end_date }}" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="show_applicants">{{ __('Show Applicants')}}</label>
                            <select name="show_applicants" id="show_applicants" class="form-control">
                                <option value="0" {{ $tender->show_applicants == 0 ? 'selected' : '' }}>{{ __('No')}}</option>
                                <option value="1" {{ $tender->show_applicants == 1 ? 'selected' : '' }}>{{ __('Yes')}}</option>
                            </select>
                        </div>
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 422) {
                        ...
                        errorMessage += '</ul>';
                        $('#error-message').html('Please correct the following errors: ' + errorMessage).show();
                    } else {
                        // Show general error message
                        $('#error-message').text('Failed to update tender. ' + (xhr.responseJSON?.message || 'Please try again.')).show();
                    }
